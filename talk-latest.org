...
* Why is this useful?
  ...
  code. The programmer is supposed to ensure that undefined behaviour
  can't happen, so the compiler can assume that any program it's given
  does not exhibit undefined behaviour. This is not without risks, but
  this can actually be really useful, and more accurately reflect
  programmer intent.

...
* How powerful is undefined behaviour?

  # Should this be here, or somewhere else?
 So, this is just a side note. It seems like this undefined behaviour
 thing is really useful. The compiler can just assume that these bad
 cases don't happen, and then it can do whatever it wants in these
 bad situations which we assume don't happen.
